...
#Verify if a sparse matrix is a solution to the N-Queens problem
function is_nq_solution(A)
  A = Matrix(A)
  n, m = size(A)
  if n != m
  ...
  for i in 1:n
    if count(x -> x == 1, A[i, :]) > 1
      return false, "More than one queen found in row $i."
    end
  end

  for j in 1:m
    if count(x -> x == 1, A[:, j]) > 1
      return false, "More than one queen found in column $j."
    end
  end

  if count(x -> x == 1, A[i, i] for i in 1:n) > 1
    return false, "More than one queen found on the main diagonal."
  end

  if count(x -> x == 1, A[i, n-i+1] for i in 1:n) > 1
    return false, "More than one queen found on the secondary diagonal."
  end

  return true, "This matrix is a Nqueens solution of order $n."
end

#Build a sparse matrix, with oreder n>8, for the N-Queens problem
function nq(n::Int64)
  A = ms(n)
  n = size(A, 1)
  if isodd(n)
    B = mod.(A, n)
    C = [el == 0 ? 1 : 0 for el in B]
    C = sparse(C)
  elseif n % 2 == 0 && (n ÷ 2) % 2 == 0
    B = mod.(A, n)
    C = [el == 0 ? 1 : 0 for el in B]
    C = sparse(C)
  elseif n % 2 == 0 && (n ÷ 2) % 2 != 0
